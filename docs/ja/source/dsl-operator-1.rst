...
ここではマスタ確認演算子を使って、 :doc:`example-app` のバッチ処理仕様 :ref:`example-app-process-join-store` に対応する処理を実装します。
...
この処理は演算子メソッド ``checkStore`` として、以下のように定義します。
...
ここではマスタ結合演算子を使って、 :doc:`example-app` のバッチ処理仕様 :ref:`example-app-process-join-item` に対応する処理を実装します。
...
この処理は演算子メソッド ``joinItemInfo`` として、以下のように定義します。
...
なお、現時点の演算子メソッド ``joinItemInfo`` の定義では結合条件は等価結合（指定した結合キーの値が一致している）条件となりますが、
このチュートリアルの後半でこの結合条件を非等価結合条件に変更する手順を説明していきます。
...
ここでは単純集計演算子を使って :doc:`example-app` のバッチ処理仕様 :ref:`example-app-process-summarize` に対応する処理を実装します。
:ref:`example-app-data-schema-joined_sales_info` に対してカテゴリコードでグループ化して、それぞれの販売数量と売上金額の合計を求めます。

この処理は演算子メソッド ``summarizeByCategory`` として、以下のように定義します。
...
ここでは更新演算子を使って、 :doc:`example-app` のバッチ処理仕様 :ref:`example-app-process-errorinfo` に対応する処理を実装します。
:ref:`example-app-data-schema-error_record` に対してエラーメッセージを設定します。

この処理は演算子メソッド ``setErrorMessage`` として、以下のように定義します。
...
マスタ結合演算子やいくつかの結合演算子ではマスタ選択用の **補助演算子** であるマスタ選択演算子を利用することで、
任意の結合条件を指定する非等価結合条件を利用することができます。
補助演算子は単体で演算子としては機能せず、他の演算子と組み合わせて利用する演算子の機能です。

ここではマスタ選択演算子を使って、 `マスタ結合演算子`_ で説明した演算子メソッド ``joinItemInfo`` に対して、
:doc:`example-app` のバッチ処理仕様 :ref:`example-app-process-join-item` に対応する次の非等価結合条件を指定します。

* 売上明細の「売上日時」が、商品マスタの「マスタ適用開始日」と「マスタ適用終了日」の範囲内にあるレコードを選択する

この処理は演算子メソッド ``selectAvailableItem`` として、以下のように定義します。
...
第1引数にはマスタデータを表すデータモデルオブジェクトを要素に取る **リスト型の引数** となる点が `マスタ確認演算子`_ や `マスタ結合演算子`_ とは異なります。
第2引数にトランザクションデータとなるデータモデルクラスを指定します。
...
ここでは `マスタ結合演算子`_ で説明した演算子メソッド ``joinItemInfo`` に対して、以下のように定義します。
